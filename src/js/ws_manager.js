...
const SERVICE_LOG_LEVEL = SERVICE_LOG_DEBUG
    ? SERVICE_LOG_LEVEL_DEBUG
    : SERVICE_LOG_LEVEL_DEFAULT
const SERVICE_MIN_LISTEN_PORT = 10101

const ERROR_WALLET_EXEC = `Failed to start ${
    config.walletServiceBinaryFilename
}. Set the path to ${
    config.walletServiceBinaryFilename
} properly in the settings tab.`
const ERROR_WALLET_PASSWORD =
    'Failed to load your wallet, please check your password'
const ERROR_WALLET_IMPORT =
    'Import failed, please check that you have entered all information correctly'
const ERROR_WALLET_CREATE =
    'Wallet can not be created, please check your input and try again'
const ERROR_RPC_TIMEOUT =
    'Unable to communicate with selected node, please try again in a few seconds or switch to another node address'
const INFO_FUSION_DONE =
    'Wallet optimization completed, your balance may appear incorrect for a while.'
const INFO_FUSION_SKIPPED =
    'Wallet already optimized. No further optimization is needed.'
const ERROR_FUSION_FAILED =
    'Unable to optimize your wallet, please try again in a few seconds'
...
var WalletShellManager = function() {
    ...
    this.serviceArgsDefault = [
        '--rpc-password',
        settings.get('service_password')
    ]
    this.walletConfigDefault = {
...
WalletShellManager.prototype.init = function() {
    ...
    this.getUnusedPort()
        ...
        .catch(err => {
            log.error(
                'Unable to find a port to listen to, please check your firewall settings'
            )
...
WalletShellManager.prototype.startService = function(
    walletFile,
    password,
    onError,
    onSuccess,
    ...
    if (null !== this.serviceLastPid) {
        ...
        log.debug(
            `Trying to clean up old/stalled process, target pid: ${
                this.serviceLastPid
            }`
        )
        try {
    ...
    let serviceArgs = this.serviceArgsDefault.concat([
        ...
        0,
        '--log-file',
        path.join(remote.app.getPath('temp'), 'ts.log'), // macOS failed without this
        '--address'
    ])
        ...
        (error, stdout, stderr) => {
            ...
            if (error) {
                ...
                if (
                    stdout &&
                    stdout.length &&
                    stdout.indexOf(config.addressPrefix) !== -1
                ) {
                    let trimmed = stdout.trim()
                    let walletAddress = trimmed.substring(
                        trimmed.indexOf(config.addressPrefix),
                        trimmed.length
                    )
                    if (walletAddress.split(':').length > 1) {
...
WalletShellManager.prototype._spawnService = function(
    walletFile,
    password,
    onError,
    onSuccess,
    ...
    if (configFile) {
        ...
        if (configFormat === 'json') {
            childProcess.execFile(
                this.serviceBin,
                serviceArgs.concat(['--save-config', configFile]),
                error => {
    ...
    function testConnection(retry) {
        wsm.serviceApi
            ...
            .catch(err => {
                ...
                if (!serviceDown && retry === 10 && onDelay)
                    onDelay(
...
WalletShellManager.prototype.genIntegratedAddress = function(
    paymentId,
    address
) {
    let wsm = this
...
WalletShellManager.prototype.createWallet = function(walletFile, password) {
    ...
    return new Promise((resolve, reject) => {
        let serviceArgs = wsm.serviceArgsDefault.concat([
            ...
            0,
            '--log-file',
            path.join(remote.app.getPath('temp'), 'ts.log')
        ])
        childProcess.execFile(
            wsm.serviceBin,
            serviceArgs,
...
WalletShellManager.prototype.importFromKeys = function(
    walletFile,
    password,
    viewKey,
    spendKey,
    ...
    return new Promise((resolve, reject) => {
        ...
        if (scanHeight >= 0)
            serviceArgs = serviceArgs.concat(['--scan-height', scanHeight])
...
WalletShellManager.prototype.importFromSeed = function(
    walletFile,
    password,
    mnemonicSeed,
    scanHeight
...
WalletShellManager.prototype._fusionGetMinThreshold = function(
    threshold,
    minThreshold,
    maxFusionReadyCount,
    counter
    ...
    return new Promise((resolve, reject) => {
        ...
        threshold =
            threshold ||
            parseInt(wsession.get('walletUnlockedBalance'), 10) * 100 + 1
        ...
        wsm.serviceApi
            .estimateFusion({ threshold: threshold })
            .then(res => {
                // nothing to optimize
                if (counter === 0 && res.fusionReadyCount === 0)
                    return resolve(0)
                // stop at maxThreshCheckIter or when threshold too low
                if (counter > maxThreshCheckIter || threshold < 10)
...
WalletShellManager.prototype._fusionSendTx = function(threshold, counter) {
    ...
    return new Promise((resolve, reject) => {
        ...
        wtime(2400).then(() => {
            ...
            wsm.serviceApi
                .sendFusionTransaction({ threshold: threshold })
                .then(resp => {
                    wsm.fusionTxHash.push(resp.transactionHash)
                    counter += 1
                    return resolve(
                        wsm._fusionSendTx(threshold, counter).then(resp => {
                            return resp
                        })
                    )
                })
                .catch(err => {
                    if (typeof err === 'string') {
                        if (
                            !err
                                .toLocaleLowerCase()
                                .includes('index is out of range')
                        ) {
                            log.debug(err)
                            return reject(new Error(err))
                        }
    ...
    if (state === 0) {
        ...
        setImmediate(() => {
            ...
            setTimeout(() => {
                ...
                this.serviceProcess = childProcess.spawn(
                    this.serviceBin,
                    this.serviceActiveArgs
                )
                // store new pid
