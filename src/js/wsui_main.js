...
const WalletShellDaemonManager = require("./ws_daemon_manager");
...
const wsdmanager = new WalletShellDaemonManager();
const sessConfig = {
...
const ADDRESS_BOOK_DEFAULT_PATH = path.join(
    ADDRESS_BOOK_DIR,
    "/SharedAddressBook.json"
);
...
function populateElementVars() {
    ...
    // generics
    genericBrowseButton = document.querySelectorAll(
        ".path-input-button:not(.d-opened"
    );
    genericFormMessage = document.getElementsByClassName("form-ew");
    genericEnterableInputs = document.querySelectorAll(
        ".section input:not(.noenter)"
    );
    genericEditableInputs = document.querySelectorAll(
    ...
    settingsInputExcludeOfflineNodes = document.getElementById(
        "pubnodes-exclude-offline"
    );
    ...
    overviewWalletCloseButton = document.getElementById(
        "button-overview-closewallet"
    );
    overviewPaymentIdGen = document.getElementById("payment-id-gen");
    overviewIntegratedAddressGen = document.getElementById(
        "integrated-wallet-gen"
    );
    overviewWalletRescanButton = document.getElementById(
        "button-overview-rescan"
    );
    // addressbook page
    addressBookInputName = document.getElementById("input-addressbook-name");
    addressBookInputWallet = document.getElementById(
        "input-addressbook-wallet"
    );
    addressBookInputPaymentId = document.getElementById(
        "input-addressbook-paymentid"
    );
    addressBookInputUpdate = document.getElementById(
        "input-addressbook-update"
    );
    ...
    walletOpenInputNode = document.getElementById(
        "input-settings-node-address"
    );
    ...
    walletCreateInputPassword = document.getElementById(
    ...
    // import wallet seed
    importSeedButtonImport = document.getElementById(
        "button-import-seed-import"
    );
    ...
    importSeedInputPassword = document.getElementById(
        "input-import-seed-password"
    );
    importSeedInputMnemonic = document.getElementById("key-import-seed");
    importSeedInputScanHeight = document.getElementById(
        "key-import-seed-height"
    );
    // tx page
    // transaction
...
function initSectionTemplates() {
    ...
    for (var i = 0; i < importLinks.length; i++) {
        let template =
            importLinks[i].import.getElementsByTagName("template")[0];
...
function changeSection(sectionId, targetRedir) {
    ...
    let finalButtonTarget =
        finalTarget === "section-welcome" ? "section-overview" : finalTarget;
    let newActiveNavbarButton = document.querySelector(
        `.navbar button[data-section="${finalButtonTarget}"]`
    );
    ...
    if (
        finalTarget === "section-overview-load" &&
    ...
    let currentButton = document.querySelector(
        `button[data-section="${finalButtonTarget}"]`
    );
    if (currentButton) {
...
function initNodeSelection(nodeAddr) {
    ...
    if (!selectedLabel.length) {
        if (remoteNodes.length) {
            ...
            let opt = walletOpenInputNode.querySelector(
                'option[value="' + selected.host + '"]'
            );
            opt.setAttribute("selected", true);
...
function initSettingVal(values) {
    ...
    if (values) {
        ...
        settings.set(
            "pubnodes_exclude_offline",
            values.pubnodes_exclude_offline
        );
    }
    ...
    settingsInputExcludeOfflineNodes.checked = settings.get(
        "pubnodes_exclude_offline"
    );
...
function showInitialPage() {
    ...
    if (versionInfo) versionInfo.innerHTML = `v${remote.app.getVersion()}`;
    let tsVersionInfo = document.getElementById("turtleServiceVersion");
    if (tsVersionInfo)
        tsVersionInfo.innerHTML = config.walletServiceBinaryVersion;
}
...
function handleSettings() {
    settingsButtonSave.addEventListener("click", function () {
        formMessageReset();
        let serviceBinValue = settingsInputServiceBin.value
            ? settingsInputServiceBin.value.trim()
            : "";
        let timeoutValue = settingsInputTimeout.value
            ? parseInt(settingsInputTimeout.value, 10)
            : 30;
        ...
        if (!serviceBinValue.length) {
            formMessageSet(
                "settings",
                "error",
                `Settings can't be saved, please enter correct values`
            );
            return false;
        }
        ...
        if (!wsutil.isRegularFileAndWritable(serviceBinValue)) {
            formMessageSet(
                "settings",
                "error",
                `Unable to find ${config.walletServiceBinaryFilename}, please enter the correct path`
            );
            return false;
        }
        ...
        if (timeoutValue < 30 || timeoutValue > 120) {
            formMessageSet(
                "settings",
                "error",
                `Timeout value must be between 30 and 120`
            );
            return false;
        }
        ...
        formMessageReset();
        let goTo = wsession.get("loadedWalletAddress").length
            ? "section-overview"
            : "section-welcome";
...
function initAddressCompletion(data) {
    var addresses = [];
    if (data) {
        addresses = Object.entries(data).map(
            ([k, v]) =>
                `${v.name}###${v.address}###${v.paymentId ? v.paymentId : ""}`
        );
    }
    ...
    window.COMPLETION_ADDRBOOK = new autoComplete({
        ...
        renderItem: function (item, search) {
            ...
            var waddr = spl[1];
            var wpayid = spl[2];
            return `<div class="autocomplete-suggestion" data-paymentid="${wpayid}" data-val="${waddr}">${wname.replace(
                re,
                "<b>$1</b>"
            )}<br><span class="autocomplete-wallet-addr">${waddr.replace(
                re,
                "<b>$1</b>"
            )}<br>Payment ID: ${
                wpayid ? wpayid.replace(re, "<b>$1</b>") : "N/A"
            }</span></div>`;
        },
        onSelect: function (e, term, item) {
            document.getElementById("input-send-payid").value =
                item.getAttribute("data-paymentid");
        },
    });
}
...
function handleAddressBook() {
            ...
            (err) => {
                if (err) {
                    ...
                    const oldBook = new Store({
                        name: "AddressBook",
                        encryptionKey: config.addressBookObfuscateEntries
                    ...
                    Object.keys(oldBook.get()).forEach((hash) => {
                        ...
                        let entryHash = wsutil.fnvhash(
                            item.address + item.paymentId
                        );
                    ...
                    setTimeout(() => {
                        ...
                        fs.rename(
                            oldAddressBook,
                            oldAddressBook + ".deprecated.txt",
                            (err) => {
                                if (err)
                                    console.error(
                                        "Failed to rename old addressbook"
                                    );
                            }
                        );
                    }, 500);
    ...
    function renderList(data) {
        if (!window.ABGRID) {
            ...
            gridOptions.onGridReady = function () {
                ...
                let agPanel = document.querySelector(
        ...
        <div class="div-transactions-panel">
                 ...
                 <div class="addressBookDetail">
                     ...
                     <div class="addressBookDetail-data">
                         <dl>
                             <dt>Name:</dt>
                             <dd data-cplabel="Wallet Name" class="tctcl" title="click to copy">${
                                 data.value.name
                             }</dd>
                             <dt>Wallet Address:</dt>
                             <dd data-cplabel="Wallet address" class="tctcl" title="click to copy">${
                                 data.value.address
                             }</dd>
                             <dt>Payment Id:</dt>
                             <dd  data-cplabel="Payment ID" class="${
                                 data.value.paymentId ? "tctcl" : "noclass"
                             }" title="${
            data.value.paymentId ? "click to copy" : "n/a"
        }">${data.value.paymentId ? data.value.paymentId : "-"}</dd>
                         </dl>
                     </div>
                 </div>
                <div class="div-panel-buttons">
                    ${sendTrtl}
                    <button data-addressid="${
                        data.key
                    }" type="button" class="form-bt button-green ab-edit" id="button-addressbook-panel-edit">Edit</button>
                    <button data-addressid="${
                        data.key
    ...
    wsutil.liveEvent("#createNewAddressBook", "click", () => {
        ...
        if (!name || !pass) {
            ...
            formMessageSet(
                "paddressbook",
                "error",
                "Address book name & password can not be left blank!"
            );
            return;
        }
        ...
        if (wsutil.isFileExist(addrPath)) {
            ...
            formMessageSet(
                "paddressbook",
                "error",
                "Same filename exists, please use different filename!"
            );
            return;
        }
        let knownAb = settings.get("address_books", []);
    ...
    addressBookSelector.addEventListener("change", () => {
        ...
        let name =
            addressBookSelector.options[addressBookSelector.selectedIndex].text;
    ...
    wsutil.liveEvent("#loadAddressBook", "click", () => {
        formMessageReset();
        let name =
            document.getElementById("pAddressbookOpenName").value || null;
        let pass =
            document.getElementById("pAddressbookOpenPass").value || null;
        let filename =
            document.getElementById("pAddressbookOpenFilename").value || null;
    ...
    addressBookButtonSave.addEventListener("click", () => {
        formMessageReset();
        let nameValue = addressBookInputName.value
            ? addressBookInputName.value.trim()
            : "";
        let addressValue = addressBookInputWallet.value
            ? addressBookInputWallet.value.trim()
            : "";
        let paymentIdValue = addressBookInputPaymentId.value
            ? addressBookInputPaymentId.value.trim()
            : "";
        let isUpdate = addressBookInputUpdate.value
            ? addressBookInputUpdate.value
            : 0;
        ...
        if (!nameValue || !addressValue) {
            formMessageSet(
                "addressbook",
                "error",
                "Name and wallet address can not be left empty!"
            );
            return;
        }
        ...
        if (!wsutil.validateAddress(addressValue)) {
            formMessageSet(
                "addressbook",
                "error",
                `Invalid ${config.assetName} address`
            );
            return;
        }
        ...
        if (addressValue.length > config.addressLength)
        ...
        if (addressBookData.hasOwnProperty(entryHash) && !isUpdate) {
            formMessageSet(
                "addressbook",
                "error",
                "This combination of address and payment ID already exist, please enter new address or different payment id."
            );
            return;
        }
        ...
        let rowData = Object.entries(abook.data).map(([key, value]) => ({
            key,
            value,
        }));
    ...
    wsutil.liveEvent(".ab-edit", "click", function (e) {
        ...
        if (!entry) {
            ...
            const walletField = document.getElementById(
                "input-addressbook-wallet"
            );
            const payidField = document.getElementById(
                "input-addressbook-paymentid"
            );
            const updateField = document.getElementById(
                "input-addressbook-update"
            );
    ...
    wsutil.liveEvent(".ab-delete", "click", function (e) {
        ...
        let rowData = Object.entries(addressBookData.data).map(
            ([key, value]) => ({ key, value })
        );
        window.ABOPTSAPI.api.setRowData(rowData);
        let axdialog = document.getElementById("ab-dialog");
    ...
    function loadAddressBook(params) {
        ...
        if (params) {
            ...
            if (params.name === "default") {
                addressBook = new WalletShellAddressbook(
                    ADDRESS_BOOK_DEFAULT_PATH
                );
            } else {
                addressBook = new WalletShellAddressbook(
                    params.path,
        ...
        if (!currentAddressBook) {
            ...
            try {
                addressBook
                    ...
                    .then((addressData) => {
                        ...
                        let ibdata = Object.entries(abdata).map(
                            ([key, value]) => ({ key, value })
                        );
                        renderList(ibdata);
                        setTimeout(() => {
            ...
            let ibdata = Object.entries(abdata).map(([key, value]) => ({
                key,
                value,
            }));
...
function handleWalletOpen() {
    ...
    wsutil.liveEvent("#saveCustomNode", "click", (e) => {
        e.preventDefault();
        formMessageReset();
        let customNodeAddressInput =
            document.getElementById("customNodeAddress");
        let nodeAddressVal = customNodeAddressInput.value
            ? customNodeAddressInput.value.trim()
            : false;
        if (!nodeAddressVal) {
            formMessageSet(
                "customnode",
                "error",
                "Invalid node address, accepted format: &lt;domain.tld&gt;:&lt;port_number&gt; or &lt;ip_address&gt;:&lt;port_number&gt;"
            );
            return;
        }
        ...
        if (nodeAddress.length !== 2) {
            formMessageSet(
                "customnode",
                "error",
                "Invalid node address, accepted format: &lt;domain.tld&gt;:&lt;port_number&gt;<br>&lt;ip_address&gt;:&lt;port_number&gt;"
            );
            return;
        }
    ...
    walletOpenRefreshNodes.addEventListener("click", () => {
        if (!navigator.onLine) {
            wsutil.showToast(
                "Network connectivity problem detected, node list update can not be performed"
            );
            return;
        }
        ...
        if (
            !confirm(
                "Refreshing node list may take a while to complete, are you sure?"
            )
        )
            return;
        fetchNodeInfo(true);
    ...
    walletOpenButtonOpen.addEventListener("click", () => {
        ...
        if (
            !navigator.onLine &&
            !nodeAddress[0].startsWith("127") &&
            nodeAddress[0] !== "localhost"
        ) {
            formMessageSet(
                "load",
                "error",
                `Network connectivity problem detected, unable to connect to remote node`
            );
            return false;
        }
        ...
        fs.access(walletFile, fs.constants.R_OK, (err) => {
            ...
            formMessageSet(
                "load",
                "warning",
                "Accessing wallet...<br><progress></progress>"
            );
            wsmanager
                .stopService()
                .then(() => {
                    formMessageSet(
                        "load",
                        "warning",
                        "Starting wallet service...<br><progress></progress>"
                    );
                    setTimeout(() => {
                        formMessageSet(
                            "load",
                            "warning",
                            "Opening wallet, please be patient...<br><progress></progress>"
                        );
    ...
    wsutil.liveEvent(".fake-options", "click", (e) => {
        let sel = e.target.classList.contains("fake-options")
            ? e.target
            : e.target.closest(".fake-options");
...
function handleWalletRescan() {
    ...
    wsutil.liveEvent("#button-rescan-start", "click", () => {
        ...
        let scanHeight = scanHeightEl.value.length
            ? parseInt(scanHeightEl.value, 10)
            : -1;
        formMessageReset();
        if (scanHeight < 0) {
            formMessageSet(
                "rescanheight",
                "error",
                "Please specify a valid scan height"
            );
            return;
        }
        let confirmMsg =
            "Rescanning process may take a long time to complete, are you sure?";
        if (scanHeight === 0) {
            confirmMsg =
                "Setting scan height to 0 will fully reset your wallet. Rescanning process may take a long time to complete, are you sure?";
        }
        if (!confirm(confirmMsg)) return;
        wsmanager
            .rescanWallet(scanHeight)
            .then(() => {
                resetTransactions();
                wsutil.showToast(
                    "Rescan OK, your wallet will be re-synchronize"
                );
            })
            .catch(() => {
                resetTransactions();
...
function handleWalletClose() {
    overviewWalletCloseButton.addEventListener("click", (event) => {
        ...
        let htmlStr =
            '<div class="div-save-main" style="text-align: center;padding:1rem;"><i class="fas fa-spinner fa-pulse"></i><span style="padding:0px 10px;">Saving &amp; closing your wallet...</span></div>';
...
function handleWalletCreate() {
    overviewButtonCreate.addEventListener("click", () => {
        formMessageReset();
        let filePathValue = walletCreateInputPath.value
            ? walletCreateInputPath.value.trim()
            : "";
        let passwordValue = walletCreateInputPassword.value
            ? walletCreateInputPassword.value.trim()
            : "";

        // validate path
        wsutil
            .validateWalletPath(filePathValue, DEFAULT_WALLET_PATH)
            .then((finalPath) => {
                // validate password
                if (!passwordValue.length) {
                    formMessageSet(
                        "create",
                        "error",
                        `Please enter a password, creating wallet without a password will not be supported!`
                    );
                    return;
                }
                ...
                if (wsutil.isRegularFileAndWritable(finalPath)) {
                    try {
                        ...
                        formMessageSet(
                            "create",
                            "error",
                            `Unable to overwrite existing file, please enter new wallet file path`
                        );
                        return;
                    }
                }
                ...
                // create
                wsmanager
                    .createWallet(finalPath, passwordValue)
                    .then((walletFile) => {
                        settings.set("recentWallet", walletFile);
                        walletOpenInputPath.value = walletFile;
                        changeSection("section-overview-load");
                        wsutil.showToast(
                            "Wallet have been created, you can now open your wallet!",
                            12000
                        );
                    })
                    .catch((err) => {
...
function handleWalletImportKeys() {
    importKeyButtonImport.addEventListener("click", () => {
        formMessageReset();
        let filePathValue = importKeyInputPath.value
            ? importKeyInputPath.value.trim()
            : "";
        let passwordValue = importKeyInputPassword.value
            ? importKeyInputPassword.value.trim()
            : "";
        let viewKeyValue = importKeyInputViewKey.value
            ? importKeyInputViewKey.value.trim()
            : "";
        let spendKeyValue = importKeyInputSpendKey.value
            ? importKeyInputSpendKey.value.trim()
            : "";
        let scanHeightValue = importKeyInputScanHeight.value
            ? parseInt(importKeyInputScanHeight.value, 10)
            : 0;
        ...
        wsutil
            ...
            .then((finalPath) => {
                ...
                if (
                    scanHeightValue < 0 ||
                    scanHeightValue.toPrecision().indexOf(".") !== -1
                ) {
                ...
                if (!viewKeyValue.length || !spendKeyValue.length) {
                    formMessageSet(
                        "import",
                        "error",
                        "View Key and Spend Key can not be left blank!"
                    );
                    return;
                }
                ...
                wsmanager
                    ...
                    .then((walletFile) => {
                        ...
                        changeSection("section-overview-load");
                        wsutil.showToast(
                            "Wallet have been imported, you can now open your wallet!",
                            12000
                        );
                    })
                    .catch((err) => {
...
function handleWalletImportSeed() {
    importSeedButtonImport.addEventListener("click", () => {
        ...
        let filePathValue = importSeedInputPath.value
            ? importSeedInputPath.value.trim()
            : "";
        let passwordValue = importSeedInputPassword.value
            ? importSeedInputPassword.value.trim()
            : "";
        let seedValue = importSeedInputMnemonic.value
            ? importSeedInputMnemonic.value.trim()
            : "";
        let scanHeightValue = importSeedInputScanHeight.value
            ? parseInt(importSeedInputScanHeight.value, 10)
            : 0;
        // validate path
        wsutil
            .validateWalletPath(filePathValue, DEFAULT_WALLET_PATH)
...
function handleWalletExport() {
    overviewShowKeyButton.addEventListener("click", () => {
        ...
        wsmanager
            .getSecretKeys(overviewWalletAddress.value)
            .then((keys) => {
            ...
            .catch(() => {
                formMessageSet(
                    "secret",
                    "error",
                    "Failed to get key, please try again in a few seconds"
                );
            });
    });
    ...
    showkeyButtonExportKey.addEventListener("click", () => {
        ...
        if (filename) {
            wsmanager
                .getSecretKeys(overviewWalletAddress.value)
                .then((keys) => {
                    let textContent = `Wallet Address:${os.EOL}${wsession.get(
                        "loadedWalletAddress"
                    )}${os.EOL}`;
                    ...
                    try {
                        ...
                        formMessageSet(
                            "secret",
                            "success",
                            "Your keys have been exported, please keep the file secret!"
                        );
                    } catch (err) {
                        formMessageSet(
                            "secret",
                            "error",
                            "Failed to save your keys, please check that you have write permission to the file"
                        );
                    }
                })
                .catch(() => {
                    formMessageSet(
                        "secret",
                        "error",
                        "Failed to get keys, please try again in a few seconds"
                    );
                });
        }
    });
...
function handleSendTransfer() {
    ...
    sendButtonSend.addEventListener("click", () => {
        ...
        let recipientAddress = sendInputAddress.value
            ? sendInputAddress.value.trim()
            : "";
        if (
            !recipientAddress.length ||
            !wsutil.validateAddress(recipientAddress)
        ) {
            formMessageSet(
                "send",
                "error",
        ...
        let paymentId = sendInputPaymentId.value
            ? sendInputPaymentId.value.trim()
            : "";
        ...
        let total = 0;
        let amount = sendInputAmount.value
            ? parseFloat(sendInputAmount.value)
            : 0;
        if (amount <= 0 || amount < config.mininumSend) {
            formMessageSet(
                "send",
                "error",
                `Sorry, minimum amount you can send is ${config.mininumSend}`
            );
            return;
        }
        ...
        if (precision(amount) > config.decimalPlaces) {
            formMessageSet(
                "send",
                "error",
                `Amount can't have more than ${config.decimalPlaces} decimal places`
            );
            return;
        }
        ...
        if (fee < minFee) {
            formMessageSet(
                "send",
                "error",
                `Fee can't be less than ${wsutil.amountForMortal(minFee)}`
            );
            return;
        }
        ...
        if (precision(fee) > config.decimalPlaces) {
            formMessageSet(
                "send",
                "error",
                `Fee can't have more than  ${config.decimalPlaces} decimal places`
            );
            return;
        }
        ...
        const availableBalance =
            wsession.get("walletUnlockedBalance") ||
            (0).toFixed(config.decimalPlaces);
            ...
            <div class="div-transaction-panel">
                ...
                <div class="transferDetail">
                    ...
                    <dl>
                        ...
                        <dt class="${
                            paymentId.length ? "dt-ib" : "hidden"
                        }">Payment ID:</dt>
                        <dd class="${paymentId.length ? "dd-ib" : "hidden"}">${
            paymentId.length ? paymentId : "N/A"
        }</dd>
                        ...
                        <dd class="dd-ib">${nodeFee > 0 ? nodeFee : "0.00"} ${
            config.assetTicker
        }</dd>
        ...
        sendBtn.addEventListener("click", (event) => {
            ...
            formMessageSet(
                "send",
                "warning",
                "Sending transaction, please wait...<br><progress></progress>"
            );
            wsmanager
                .sendTransaction(tx)
                .then((result) => {
                    formMessageReset();
                    let txhashUrl = `<a class="external" title="view in block explorer" href="${config.blockExplorerUrl.replace(
                        "[[TX_HASH]]",
                    ...
                    if (!addressBookData.hasOwnProperty(entryHash)) {
                        ...
                        abook.data[entryHash] = newAddress;
                        wsession.set("addressBook", abook);
                        let rowData = Object.entries(abook.data).map(
                            ([key, value]) => ({ key, value })
                        );
                        window.ABOPTSAPI.api.setRowData(rowData);
                        window.ABOPTSAPI.api.deselectAll();
    ...
    sendOptimize.addEventListener("click", () => {
        ...
        if (
            !confirm(
                "You are about to perform wallet optimization. This process may take a while to complete, are you sure?"
            )
        )
            return;
        wsutil.showToast(
            "Optimization started, your balance may appear incorrect during the process",
            3000
        );
...
function handleTransactions() {
    function resetTxSortMark() {
        let sortedEl = document.querySelectorAll(
            "#transaction-lists .asc, #transaction-lists .desc"
        );
                    ...
                    <table class="custom-table" id="transactions-panel-table">
                        <tbody>
                            <tr><th scope="col">Hash</th>
                                <td data-cplabel="Tx. hash" class="tctcl">${
                                    tx.transactionHash
                                }</td></tr>
                            <tr><th scope="col">Address</th>
                                <td data-cplabel="Address" class="tctcl">${wsession.get(
                                    "loadedWalletAddress"
                                )}</td></tr>
                            <tr><th scope="col">Payment Id</th>
                                <td data-cplabel="Payment ID" class="${
                                    tx.paymentId !== "-" ? "tctcl" : "xtctl"
                                }">${tx.paymentId}</td></tr>
                            <tr><th scope="col">Amount</th>
                                <td data-cplabel="Tx. amount" class="tctcl">${
                                    tx.amount
                                }</td></tr>
                            <tr><th scope="col">Fee</th>
                                <td  data-cplabel="Tx. fee" class="tctcl">${
                                    tx.fee
                                }</td></tr>
                            <tr><th scope="col">Timestamp</th>
                                <td data-cplabel="Tx. date" class="tctcl">${
                                    tx.timestamp
                                } (${txdate})</td></tr>
                            <tr><th scope="col">Block Index</th>
                                <td data-cplabel="Tx. block index" class="tctcl">${
                                    tx.blockIndex
                                }</td></tr>
                            <tr><th scope="col">Is Base?</th>
                                <td>${tx.isBase}</td></tr>
                            <tr><th scope="col">Extra</th>
                                <td data-cplabel="Tx. extra" class="${
                                    tx.extra !== "-" ? "tctcl" : "xtctl"
                                }">${tx.extra}</td></tr>
    ...
    function exportAsCsv(mode) {
        ...
        switch (mode) {
            case "in":
                ...
                break;
            case "out":
                outData = txlist.filter((obj) => {
                    return obj.txType === "out";
                });
                outType = "outgoing";
                break;
            default:
        ...
        if (!outData.length) {
            wsutil.showToast(
                `Transaction export failed, ${outType} transactions is not available!`
            );
            if (dialog.hasAttribute("open")) dialog.close();
            return;
        }
...
function handleNetworkChange() {
    window.addEventListener("online", () => {
        ...
        if (!connectedNode.length || connectedNode.startsWith("127.0.0.1"))
            return;
...
function initHandlers() {
    ...
    for (var ei = 0; ei < sectionButtons.length; ei++) {
        ...
        sectionButtons[ei].addEventListener(
            "click",
            changeSection.bind(this, target),
            false
        );
    }
    // misc shortcut
    ...
    function handleBrowseButton(args) {
        ...
        if (dialogType === "saveFile") {
            dialogOpts.defaultPath = path.join(
                recentDir,
                `new_wallet_${new Date().getTime()}.${
                    config.walletFileDefaultExt
                }`
            );
    ...
    function handleFormEnter(el) {
        try {
            clearTimeout(window.enterHandler);
        } catch (_e) {}
    ...
    setTimeout(() => {
        ...
        wsutil.liveEvent(".togpass", "click", (e) => {
            let tg = e.target.classList.contains(".togpas")
                ? e.target
                : e.target.closest(".togpass");
            if (!tg) return;
            let targetId = tg.dataset.pf || null;
            if (!targetId) return;
            let target = document.getElementById(targetId);
            target.type = target.type === "password" ? "text" : "password";
            tg.firstChild.dataset.icon =
                target.type === "password" ? "eye-slash" : "eye";
        });
        // context menu
        ...
        for (var i = 0; i < genericBrowseButton.length; i++) {
            ...
            let args = {
                ...
                targetName: genericBrowseButton[i].dataset.fileobj
                    ? genericBrowseButton[i].dataset.fileobj
                    : "",
                targetInput: document.getElementById(targetInputId),
                targetButton: genericBrowseButton[i].id,
            };
            genericBrowseButton[i].addEventListener(
                "click",
                handleBrowseButton.bind(this, args)
            );
        }
        ...
        wsutil.liveEvent("dialog input:not(.noenter)", "keyup", (e) => {
            ...
            window.enterHandler = setTimeout(() => {
                if (key === "Enter") {
                    ...
                    let target = section.querySelector(
                        "button:not(.notabindex)"
                    );
                    if (target) {
                        let event = new MouseEvent("click", {
                            view: window,
        ...
        overviewIntegratedAddressGen.addEventListener(
            "click",
            showIntegratedAddressForm
        );
        wsutil.liveEvent("#doGenIntegratedAddr", "click", () => {
            formMessageReset();
            let genInputAddress = document.getElementById("genInputAddress");
            let genInputPaymentId =
                document.getElementById("genInputPaymentId");
            let outputField = document.getElementById(
                "genOutputIntegratedAddress"
            );
            let addr = genInputAddress.value
                ? genInputAddress.value.trim()
                : "";
            let pid = genInputPaymentId.value
                ? genInputPaymentId.value.trim()
                : "";
            outputField.value = "";
            ...
            if (!addr.length || !pid.length) {
                formMessageSet(
                    "gia",
                    "error",
                    "Address & Payment ID is required"
                );
                return;
            }
            ...
            if (addr.length > config.addressLength) {
                formMessageSet(
                    "gia",
                    "error",
                    `Only standard ${config.assetName} address are supported`
                );
                return;
            }
        ...
        wsutil.liveEvent("textarea.ctcl, input.ctcl", "click", (event) => {
            ...
            let cpnotice = cplabel
                ? `${cplabel} copied to clipboard!`
                : "Copied to clipboard";
...
function initKeyBindings() {
    ...
    Mousetrap.bind(["ctrl+o", "command+o"], () => {
        ...
        if (walletOpened) {
            wsutil.showToast(
                "Please close current wallet before opening another wallet!"
            );
            return;
        }
    ...
    Mousetrap.bind(["ctrl+n", "command+n"], () => {
        ...
        if (walletOpened) {
            wsutil.showToast(
                "Please close current wallet before creating/importing new wallet"
            );
            return;
        }
    ...
    Mousetrap.bind(["ctrl+t", "command+t"], () => {
        ...
        if (!walletOpened) {
            wsutil.showToast(
                "Please open your wallet to view your transactions"
            );
            return;
        }
...
function fetchFromRaw() {
    ...
    for (const n of nodes) {
        let feeLabel =
            parseInt(n.fee, 10) > 0
                ? `Fee: ${wsutil.amountForMortal(n.fee)} ${config.assetTicker}`
                : "FREE";
        tested_nodes.push({
            host: `${n.url}:${n.port}`,
...
function fetchNodeInfo(force) {
    ...
    walletOpenNodeLabel.innerHTML =
        '<i class="fas fa-sync fa-spin"></i> Updating node list, please wait...';
            ...
            return fetchWait(url)
                .then((response) => {
                    if (response.hasOwnProperty("status")) {
                        // fake/timeout response
                        try {
                            window.FETCHNODESIG.abort();
                        } catch (e) {}
                        return response;
                    } else {
                ...
                .then((json) => {
                    ...
                    if (json.status === "KO") {
                        ...
                        feeAmount =
                            parseInt(json.amount, 10) > 0
                                ? `Fee: ${wsutil.amountForMortal(
                                      json.amount
                                  )} ${config.assetTicker}`
                                : "FREE";
                    }
    ...
    wsmanager
        ...
        .then(() => {
            setTimeout(function () {
                wsmanager.terminateService(true);
                try {
                    fs.unlinkSync(wsession.get("walletConfig"));
                } catch (e) {}
                win.close();
            }, 1200);
        })
