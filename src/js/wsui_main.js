...
function showIntegratedAddressForm() {
    ...
    <div class="transaction-panel">
        ...
        <div class="input-wrap">
            ...
            <textarea id="genInputAddress" class="default-textarea" placeholder="Required, put any valid ${
                config.assetTicker
            } address..">${ownAddress}</textarea>
        </div>
        <div class="input-wrap">
    ...
    window.COMPLETION_ADDRBOOK = new autoComplete({
        ...
        onSelect: function(e, term, item) {
            document.getElementById(
                'input-send-payid'
            ).value = item.getAttribute('data-paymentid')
        }
    })
}
...
function handleAddressBook() {
    ...
    function renderItem(e) {
        ...
        if (myaddress && isSynced) {
            sendTrtl = `<button data-addressid="${
                data.key
            }" type="button" class="form-bt button-green ab-send" id="button-addressbook-panel-send">Send ${
                config.assetTicker
            }</button>`
        }
...
function handleWalletOpen() {
                ...
                <div class="splitted-area splitted-2">
                    ...
                    <div class="splitted-content last">
                        <div class="input-wrap f-right">
                            ...
                            <input type="text" id="customNodeAddress" placeholder="my.example.com:${
                                config.daemonDefaultRpcPort
                            }" class="text-block" />
                            <span class="form-help">Required, example: my.example-domain.com:${
                                config.daemonDefaultRpcPort
                            }, 123.123.123.123:${
            config.daemonDefaultRpcPort
        }</span>
    ...
    wsutil.liveEvent('#saveCustomNode', 'click', e => {
        e.preventDefault()
        formMessageReset()
        let customNodeAddressInput = document.getElementById(
            'customNodeAddress'
        )
        ...
        wsmanager
            ...
            .then(keys => {
                ...
                if (keys.mnemonicSeed && keys.mnemonicSeed.length > 1) {
                    ...
                    showkeyInputSeed.value = `- Mnemonic seed is not available for this wallet -${
                        os.EOL
                    }You still can restore your wallet using private keys shown above.`
        ...
        if (filename) {
            wsmanager
                ...
                .then(keys => {
                    ...
                    textContent += `${os.EOL}View Secret Key:${os.EOL}${
                        keys.viewSecretKey
                    }${os.EOL}`
                    textContent += `${os.EOL}Spend Secret Key:${os.EOL}${
                        keys.spendSecretKey
                    }${os.EOL}`
                    if (keys.mnemonicSeed && keys.mnemonicSeed.length > 1) {
                        textContent += `${os.EOL}Mnemonic Seed:${os.EOL}${
                            keys.mnemonicSeed
                        }${os.EOL}`
                    }
                    try {
            ...
            let columnDefs = [
                {
                    ...
                    cellRenderer: function(params) {
                        let out = `
                            ...
                            <p class="tx-ov-info">Tx. Hash: ${
                                params.data.transactionHash
                            }</p>
                            <p class="tx-ov-info">Payment ID: ${
                ...
                },
                {
                    ...
                    cellRenderer: function(params) {
                        return `<span data-txType="${params.data.txType}">${
                            params.data.amount
                        }</span>`
                    },
                    suppressFilter: true,
