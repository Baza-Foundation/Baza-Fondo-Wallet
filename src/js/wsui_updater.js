...
let LAST_LOCAL_DAEMON_BLOCK_COUNT = 0;
let LAST_NETWORK_BLOCK_COUNT = 0;
...
function updateSyncProgress(data) {
    ...
    switch (knownBlockCount) {
        case syncStatus.NET_ONLINE:
            ...
            // connection status
            connInfoDiv.innerHTML =
                "Connection restored, resuming sync process...";
        ...
        case syncStatus.NET_OFFLINE:
            ...
            // connection status
            connInfoDiv.innerHTML =
                "Synchronization paused, please check your network connection!";
            connInfoDiv.classList.remove("empty");
        ...
        case syncStatus.RESET:
            if (!connInfoDiv.innerHTML.startsWith("Connected")) {
                let connStatusText = `Connected to: <strong>${wsession.get(
                    "connectedNode"
                )}</strong>`;
                let connNodeFee = wsession.get("nodeFee");
                if (connNodeFee > 0) {
                    connStatusText += ` | Node fee: <strong>${connNodeFee.toFixed(
                        config.decimalPlaces
                    )} ${config.assetTicker}</strong>`;
                }
        ...
        case syncStatus.NODE_ERROR:
            ...
            // connection status
            connInfoDiv.innerHTML =
                "Connection failed, try switching to another Node, close and reopen your wallet";
            connInfoDiv.classList.remove("empty");
            connInfoDiv.classList.add("conn-warning");
            wsession.set("connectedNode", "");
            brwin.setProgressBar(-1);
        ...
        default:
            ...
            if (blockCount + 1 >= knownBlockCount && knownBlockCount !== 0) {
                ...
                // status text
                statusText = `SYNCED WALLET ${statusText}`;
                syncInfoBar.textContent = statusText;
                // status icon
                ...
                syncDiv.className = "syncing";
                // status text
                statusText = `SYNCING WALLET ${statusText}`;
                if (blockSyncPercent < 100)
                    statusText += ` (${blockSyncPercent}%)`;
                syncInfoBar.textContent = statusText;
                // status icon
                iconSync.setAttribute("data-icon", "sync");
                iconSync.classList.add("fa-spin");
                ...
                wsession.set("synchronized", false);
                let taskbarProgress = +(
                    parseFloat(blockSyncPercent) / 100
                ).toFixed(2);
...
function updateDaemonSyncProgress(data) {
    const iconSync = document.getElementById("navbar-icon-sync");
    let blockCount = data.displayLocalDaemonBlockCount;
    let knownBlockCount = data.displayNetworkBlockCount;
    let blockSyncPercent = data.daemonSyncPercent;
    let statusText = "";

    switch (knownBlockCount) {
        case daemonSyncStatus.NET_ONLINE:
            // sync status text
            statusText = "RESUMING DAEMON SYNC...";
            syncInfoBar.innerHTML = statusText;
            // sync info bar class
            syncDiv.className = "syncing";
            // sync status icon
            ...
            brwin.setProgressBar(-1);
            break;
        case daemonSyncStatus.NET_OFFLINE:
            // sync status text
            statusText = "PAUSED, NETWORK DISCONNECTED";
            syncInfoBar.innerHTML = statusText;
            // sync info bar class
        ...
        case daemonSyncStatus.RESET:
            ...
            brwin.setProgressBar(-1);
            break;
        case daemonSyncStatus.NODE_ERROR:
            // status info bar class
            syncDiv.className = "failed";
            // sync status text
            statusText = "NODE ERROR";
        ...
        default:
            ...
            if (blockCount + 1 >= knownBlockCount && knownBlockCount !== 0) {
                ...
                syncDiv.classList = "synced";
                // status text
                statusText = `SYNCED DAEMON ${statusText}`;
                syncInfoBar.textContent = statusText;
                // status icon
                iconSync.setAttribute("data-icon", "check");
                iconSync.classList.remove("fa-spin");
                ...
                syncDiv.className = "syncing";
                // status text
                statusText = `SYNCING DAEMON ${statusText}`;
                if (blockSyncPercent < 100)
                    statusText += ` (${blockSyncPercent}%)`;
                syncInfoBar.textContent = statusText;
                // status icon
            ...
            LAST_LOCAL_DAEMON_BLOCK_COUNT = blockCount;
            LAST_NETWORK_BLOCK_COUNT = knownBlockCount;
            break;
    }
...
function fusionCompleted() {
    ...
    wsutil.showToast(
        "Optimization completed. You may need to repeat the process until your wallet is fully optimized.",
        5000
    );
}

function updateBalance(data) {
    const balanceAvailableField = document.querySelector(
...
function updateTransactions(result) {
    ...
    if (!txlistExisting.length && !blockItems.length) {
        ...
        document
            .getElementById("transaction-export")
    ...
    Array.from(blockItems).forEach((block) => {
        block.transactions.map((tx) => {
            if (
                tx.amount !== 0 &&
                !wsutil.objInArray(txlistExisting, tx, "transactionHash")
            ) {
    ...
    lastTxDate = `${lastTxDate.getUTCFullYear()}-${
        lastTxDate.getUTCMonth() + 1
    }-${lastTxDate.getUTCDate()}`;
    ...
    let notify = true;
    if (
        lastTxDate !== currentDate ||
        newTxAmount < 0 ||
        rememberedLastHash === newLastHash
    ) {
        notify = false;
    }
...
function showFeeWarning(fee) {
    ...
    let htmlStr = `
        <h5>Fee Info</h5>
        <p>You are connected to a public node (${settings.get(
            "node_address"
        )}) that charges a fee to send transactions.<p>
        <p>The fee for sending transactions is: <strong>${fee.toFixed(
            config.decimalPlaces
        )} ${config.assetTicker} </strong>.<br>
...
function updateQr(address) {
    ...
    if (!address) {
        ...
        try {
            clearInterval(window.backupReminderTimer);
        } catch (_e) {}
        return;
    }
    ...
    for (var i = 0; i < allFormInputs.length; i++) {
        ...
        if (el.dataset.initial) {
            if (!el.dataset.noclear) {
                el.value = settings.has(el.dataset.initial)
                    ? settings.get(el.dataset.initial)
                    : "";
...
function updateUiState(msg) {
    ...
    switch (msg.type) {
        case "daemonBlockUpdated":
            updateDaemonSyncProgress(msg.data);
            break;
        case "blockUpdated":
            if (LAST_LOCAL_DAEMON_BLOCK_COUNT === LAST_NETWORK_BLOCK_COUNT) {
